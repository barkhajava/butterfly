...
from butterfly.logic.datasource import DataSource
...
class Mojo(DataSource):
    ...
    def index(self):
        ...
        z_ind = list(range(num_slices))
        y_ind = list(range(len(glob.glob(y_tiles))))
        x_ind = list(range(len(glob.glob(x_tiles))))
